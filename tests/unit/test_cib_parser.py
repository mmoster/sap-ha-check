...
class TestGetResources:
    ...
    def test_parses_resource_list(self, mock_exists, mock_run):
        ...
        resources_text = " ".join(result["resources"])
  ...
  Resource: SAPHanaController_S4D_HDB00 (class=ocf provider=heartbeat type=SAPHanaController)
    ...
    Meta Attributes:
      ...
      promotable=true"""
        ...
        assert result["success"] is True
        assert len(result["sap_hana"]) >= 1
        # Verify extracted HANA config contains expected attributes
        hana_config = result["sap_hana"][0] if isinstance(result["sap_hana"], list) else result["sap_hana"]
        assert "SAPHanaController" in str(hana_config)
...
class TestGetConstraints:
    ...
    def test_parses_constraint_sections(self, mock_exists, mock_run):
        ...
        assert len(result["colocation"]) >= 1
        assert len(result["order"]) >= 1
        # Verify specific constraint content from fixture
        location_text = str(result["location"])
    ...
    def test_detects_resource_discovery(self, mock_exists, mock_run):
        ...
        result = parser.get_constraints()
        assert len(result["resource_discovery"]) >= 1
        discovery_text = str(result["resource_discovery"])
        assert "never" in discovery_text.lower()
...
class TestGetStonith:
    ...
    def test_stonith_enabled_stays_none_when_property_format_has_extra_text(self, mock_exists, mock_run):
        ...
        When the property value contains extra text (e.g. parenthetical defaults),
        the parser cannot match it and enabled stays None.
        """
